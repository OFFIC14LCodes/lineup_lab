...
function normalizeStatRow(row: WeeklyStatRow): Record<string, number> {
  return {
    ...
    passingFirstDowns: normalizeNumber(row.passingFirstDowns ?? 0) ?? 0,
    sacksTaken: normalizeNumber(row.sacksTaken ?? 0) ?? 0,
    passPick6: normalizeNumber(row.passPick6 ?? 0) ?? 0,
    receptions: normalizeNumber(row.receptions) ?? 0,
    receivingFirstDowns: normalizeNumber(row.receivingFirstDowns ?? 0) ?? 0,
    receivingTds: normalizeNumber(row.receivingTds) ?? 0,
    receivingYards: normalizeNumber(row.receivingYards) ?? 0,
    recTd40p: normalizeNumber(row.recTd40p ?? 0) ?? 0,
    recTd50p: normalizeNumber(row.recTd50p ?? 0) ?? 0,
    rushingFirstDowns: normalizeNumber(row.rushingFirstDowns ?? 0) ?? 0,
    rushingTds: normalizeNumber(row.rushingTds) ?? 0,
    rushingYards: normalizeNumber(row.rushingYards) ?? 0,
    rushTd40p: normalizeNumber(row.rushTd40p ?? 0) ?? 0,
    rushTd50p: normalizeNumber(row.rushTd50p ?? 0) ?? 0,
